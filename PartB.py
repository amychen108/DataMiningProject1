...
def fit_gd_multi(x_tr, y_tr, lr, ep, m0=1.0, b0=1.0):
...
def run_set_multi(set_n, xtr, ytr, xte, yte, lr, ep):
    m, b, loss = fit_gd_multi(xtr, ytr, lr, ep)
    ytr_pred = xtr@m+b
    yte_pred = xte@m+b

    final_mse_direct = np.mean((xtr @ m + b - ytr) ** 2)
    ...
    print()
    print("Len of loss:", len(loss))
    print("Final_mse_direct = ", final_mse_direct)
    print("Difference = ", abs(loss[-1]-final_mse_direct))
    print("loss[0] =", loss[0])
    print("loss[1] =", loss[1])
    print("loss[-1] =", loss[-1])
    ...
    # print("y_train var :", y_train.var())
    print()
    print()


    # step = max(1, len(loss)//2000)
    # xs = np.arange(0, len(loss), step)
    # ys = np.array(loss)[xs]
    plt.figure()
    # plt.plot(xs, ys)
    plt.plot(loss)
...
run_set_multi("Set2_standardize", xtr_std, y_train, xte_std, y_test, 0.05, 250)

#Set 2.2: raw
run_set_multi("Set2_raw", x_train, y_train, x_test, y_test, 1e-7, 30000)
